...
add_additional_track <- function(p, track_data, offset = 0.5) {
  track_data <- track_data %>% mutate(y_numeric = as.numeric(seqid) - offset)
  p + geom_segment(
    data = track_data,
    aes(x = start, xend = end, y = y_numeric, yend = y_numeric, color = region_type),
    linewidth = 2)
}
...
if (!is.infinite(opts$max_chromosomes)) {
  chrom_limits <- chrom_limits %>% arrange(desc(chrom_length)) %>% head(opts$max_chromosomes)
}
# if (!is.infinite(opts$max_chromosomes)) {
#   chrom_limits <- chrom_limits %>%
...
if (opts$facet_plot) {
  ...
  fname <- paste0("gene_distribution_facet_", opts$facet_mode, "_", opts$facet_scales)
